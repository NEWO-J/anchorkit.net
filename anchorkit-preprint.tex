...
\begin{itemize}
  ...
  \item A custom Solana program storing Merkle roots in a chunked linked-list PDA structure with fully offline verification
...
\paragraph{Client-side defenses.} The SDK inspects \texttt{Build.TAGS} for \texttt{test-keys}, scans for root management artifacts, and implements programmatic SPKI SHA-256 certificate pinning on \texttt{HttpsURLConnection}, bypassing any device-level \texttt{network\_security\_config.xml} overrides.
...
At midnight UTC, the nightly batch filters records by \texttt{day == yesterday}, sorts by \texttt{hash\_id}, writes a compressed JSONL archive to S3 with a sharded lookup index (enabling $O(1)$ hash-to-day lookup), constructs a Merkle tree over the ordered hash sequence using a Rust implementation, and posts the resulting root to the Solana program via the \texttt{add\_merkle\_root} instruction.

The Solana program stores roots in program-derived accounts organized as a chunked linked list. Each entry contains the 32-byte Merkle root, a UTF-8 date string, and a Unix timestamp. The borsh serialization is deterministic and language-independent, allowing verification using any borsh library.
...
\paragraph{Solana program authority key compromise.} Compromise of the authority key permits posting of false Merkle roots for future dates only. Under Solana's consensus assumption, existing on-chain records remain intact, and the compromise is detectable by any party monitoring the program account.
...
\paragraph{Multi-chain anchoring.} Posting Merkle roots to multiple independent blockchains would reduce dependence on Solana's continued operation and provide defense against chain-level governance failures.

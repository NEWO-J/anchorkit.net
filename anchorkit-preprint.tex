...
\[
  ...
  \end{cases}
\]
with $v_k = R$. Let $i^*$ denote the leaf position implied by the direction sequence $(d_1, \ldots, d_k)$, and let $w_0 = h_{i^*}$ be the honest leaf at position $i^*$ in $S$. Define $w_j$ by the same recurrence with the same siblings $(s_1, \ldots, s_k)$: $w_j$ is the node obtained by recomputing the path from $h_{i^*}$ using $\pi^*$'s siblings. Since the Merkle tree over $S$ is consistent, $w_k = R$.

We have $v_k = w_k = R$ and $v_0 = h^* \neq h_{i^*} = w_0$ (since $h^* \notin S$). Therefore there exists a smallest index $\ell \in \{1, \ldots, k\}$ such that $v_\ell = w_\ell$ but $v_{\ell-1} \neq w_{\ell-1}$. At level $\ell$, both paths apply the same SHA-256 computation (same sibling $s_\ell$, same direction $d_\ell$) to distinct inputs $v_{\ell-1} \neq w_{\ell-1}$ and obtain the same output $v_\ell = w_\ell$. This is a SHA-256 collision. A $\ppt$ reduction $\calB$ that runs $\calA$, finds the smallest such $\ell$, and outputs $(v_{\ell-1}, w_{\ell-1})$ breaks SHA256-CR with probability $\epsilon$. By Assumption~\ref{asm:sha256}, $\epsilon \leq \negl(\lambda)$.
\end{proof}
...
\bibitem{proofmode}
Guardian Project.
\newblock ProofMode: Authenticating Media from the Field.
\newblock \url{https://proofmode.org}, 2017.

\bibitem{starling}
Starling Lab.
\newblock Starling Capture: Authenticity Infrastructure for Visual Evidence.
\newblock \url{https://starlinglab.org}, 2021.

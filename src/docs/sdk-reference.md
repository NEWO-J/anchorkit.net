...
This is a suspending function; call it from a coroutine scope.
...
Download a `PortableProof` for a hash that has already been anchored. Store the returned object in your own database, as it is permanently verifiable without contacting AnchorKit.
...
A self-contained proof bundle. Obtained via `downloadProof`; verified via `verifyLocally`. Store this in your own database, as it remains permanently verifiable without AnchorKit.
...
if (result.valid) {
    // Hash is confirmed on-chain without contacting any AnchorKit server
} else {

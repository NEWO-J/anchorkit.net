...
export default function DevelopersPage() {
  ...
  const applyKeyData = (data: KeyData) => setKeyData(data);
  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-8">
        <h1 className="font-['DM_Sans',sans-serif] font-bold text-2xl text-white">Developers</h1>
        <p className="font-['DM_Sans',sans-serif] text-sm text-white/30 mt-1">Manage your API key, webhooks, and review security events.</p>
      </div>
      ...
      <div className="flex border-b border-white/[0.08] mb-8">
        {tabs.map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`px-4 pb-3 pt-1 text-sm font-['DM_Sans',sans-serif] font-medium transition-colors cursor-pointer border-b-2 -mb-px
              ${tab === t.id
                ? 'text-white border-[#ff7608]'
        ...
        <div>
          ...
          {keyData && (
            <>
              ...
              <div className="mb-2">
                <p className="font-['DM_Sans',sans-serif] text-xs text-white/30 mb-2 uppercase tracking-wide">Your API Key</p>
                <div className="flex items-center gap-2">
                  <code className={`flex-1 bg-black/30 border border-white/[0.08] rounded-[6px] px-4 py-3 font-mono text-sm break-all select-all transition-colors ${keyData.key_paused ? 'text-white/25' : 'text-white/80'}`}>
                    {visible ? keyData.api_key : maskedKey}
                  </code>
                  <button onClick={() => setVisible(v => !v)} title={visible ? 'Hide' : 'Reveal'}
                    className="shrink-0 px-3 py-3 rounded-[6px] bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.08] transition-colors cursor-pointer text-white/50 hover:text-white/80">
                    {visible ? (
                ...
                <p className="font-['DM_Sans',sans-serif] text-xs text-white/25 mt-2">
                  Pass as <code className="font-mono text-white/35">api_key</code> in your SDK configuration. Keep it secret.
                </p>
              </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleRegisterWebhook} disabled={webhookLoading || !webhookUrl}
                className="shrink-0 px-4 py-2.5 rounded-[6px] bg-white/[0.06] border border-white/[0.08] font-['DM_Sans',sans-serif] text-sm text-white/60 hover:text-white/80 hover:bg-white/[0.10] transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed">
                {webhookLoading ? '…' : 'Add'}
              </button>
            </div>
            {webhookSecret && (
              <div className="mt-3 px-3 py-2.5 rounded-[6px] bg-black/30 border border-white/[0.08]">
      ...
      {tab === 'audit' && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="font-['DM_Sans',sans-serif] text-white/15 text-xs mt-1">
            Will show key operations, login events, and webhook deliveries.
          </p>
        </div>
      )}
    </div>

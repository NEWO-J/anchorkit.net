...
function useMidnightUTCCountdown(): string {
  ...
  React.useEffect(() => {
    function calc() {
      ...
      let secs = Math.max(0, Math.floor((midnight.getTime() - now.getTime()) / 1000));
...
function PixelHorizon({ centerPx }: { centerPx: number }) {
  ...
  React.useEffect(() => {
    ...
    function draw() {
      ...
      const PIXEL = 5;
      const SPREAD_PX = 216; // dithering zone height in pixels
...
export default function AnchorLogPage() {
  ...
  const countdown = useMidnightUTCCountdown();
      ...
      <div className="relative w-full max-w-4xl xl:max-w-6xl 2xl:max-w-7xl">
        ...
        <div className="mb-10 text-center">
          ...
          <div className="relative max-w-lg mx-auto">
            <div
              ...
              style={{ top: '-16px', right: '-1.5rem', bottom: '-16px', left: '-1.5rem', background: 'radial-gradient(ellipse 80% 100% at center, rgba(3,0,40,0.95) 0%, rgba(3,0,40,0.6) 45%, rgba(3,0,40,0.15) 75%, transparent 100%)' }}
            />
        ...
        {state.phase === 'loaded' && state.entries.length > 0 && (
          <div className="grid grid-cols-3 border border-white/10 bg-[#030028] divide-x divide-white/10 mb-0">
            ...
            <div className="px-5 py-4 text-center">
              <p className="text-2xl font-['DM_Sans',sans-serif] font-bold text-white tabular-nums">
                {countdown || '—'}
              </p>
              <p className="text-xs text-white/40 mt-1 uppercase tracking-wide">Until Next Anchor</p>
            </div>
          </div>
        )}

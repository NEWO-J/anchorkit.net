...
function AnchorMesh({ groupRef }: { groupRef: React.RefObject<THREE.Group | null> }) {
  ...
  // Geometry objects — shared between left/right symmetrical pieces
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Bottom arc — half-torus, rotated PI around Z so it opens upward */}
...
function Scene({ targetRotation }: { targetRotation: number }) {
  ...
  return (
    <>
      ...
      {/* disableNormalPass avoids an unused G-buffer pass.
          multisampling={0} prevents MSAA FBOs from dropping the alpha channel. */}

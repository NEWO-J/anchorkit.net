...
export default function PhoneParallax() {
  ...
  React.useEffect(() => {
    function updateParallax() {
      ...
      // scroll down → image moves down
      setParallaxPx(Math.max(-60, Math.min(60, -offset * 0.18)));
    }

    function updateCards() {
      if (stageRef.current) {
        setShowCards(stageRef.current.offsetWidth >= 500);
      }
    }
  ...
  return (
    <div
      ref={containerRef}
      style={{ width: '100%', minHeight: '520px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
    >
      {/* Stage */}
      <div
        ref={stageRef}
        style={{ position: 'relative', width: 'min(700px, 100%)', height: '490px' }}
      >
        ...
        >
          ...
          <div style={{ position: 'absolute', inset: '1.2%', borderRadius: 'clamp(10px, 1.4vw, 20px)', overflow: 'hidden', background: '#000' }}>
            <img

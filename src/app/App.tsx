...
function PrimaryButton({ onClick, zr = 1 }: { onClick: () => void; zr?: number }) {
  return (
    <button
      onClick={onClick}
      className="bg-transparent border border-white/30 hover:border-white/50 rounded-[7px] px-4 py-1.5 font-['DM_Sans',sans-serif] font-medium text-[rgba(224,222,255,0.7)] hover:text-[rgba(224,222,255,0.9)] transition-all flex items-center gap-2"
      style={{ fontSize: `clamp(0.875rem, calc(1.15vw * ${zr}), 1.4rem)` }}
    >
  ...
  const textClass = ghost
    ...
    : variant === 'dark'
      ...
      : variant === 'orange'
        ? 'text-[#030028]'
        : 'text-white';
  const fontSizeStyle = zr !== 1 ? { fontSize: `clamp(0.875rem, calc(1.15vw * ${zr}), 1.4rem)` } : undefined;
  const btn = (
    <button
      onClick={onClick}
